...
{
    public class SetHeadTransformSystem : IEcsRunSystem
    {
        ...
        public void Run()
        {
            foreach (var entity in _devicesFilter)
            {
                ...
                foreach (var device in devices.Devices)
                {
                    ...
                    head.Position = device.TryGetFeatureValue(CommonUsages.devicePosition, out var position) ? position : Vector2.zero;
                    head.Rotation = device.TryGetFeatureValue(CommonUsages.deviceRotation, out var rotation) ? rotation : Quaternion.identity;
                }
            }
        }
    }
